(function() {
  ...
  angular.module('civic.pages')
    .directive('organizationsByEvidenceActivity', organizationsByEvidenceActivity)
    .controller('organizationsByEvidenceActivityController',
                organizationsByEvidenceActivityController);

  // @ngInject
  function organizationsByEvidenceActivity() {
    var directive = {
      restrict: 'E',
      scope: {
        options: '=',
  ...
  // @ngInject
  function organizationsByEvidenceActivityController($scope,
                                    $window,
                                    $rootScope,
                                    $element,
                                    d3,
                                    ...
                                    _,
                                   Stats) {
    console.log('organizationsByEvidenceActivity loaded.');
    ...
    $scope.$watch(function() {
      ...
      chart.data = _.map(data, function(value, key) {
        return {
          Organization: key,
          Count: _.reduce(value.evidence_counts, function(total, value, key) {
            return total + value;
          }, 0)
        };
      });
