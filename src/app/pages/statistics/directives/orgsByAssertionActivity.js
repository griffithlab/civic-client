(function() {
  ...
  angular.module('civic.pages')
    .directive('organizationsByAssertionActivity', organizationsByAssertionActivity)
    .controller('organizationsByAssertionActivityController',
                organizationsByAssertionActivityController);

  // @ngInject
  function organizationsByAssertionActivity() {
    var directive = {
      restrict: 'E',
      scope: {
        options: '=',
  ...
  // @ngInject
  function organizationsByAssertionActivityController($scope,
                                    $window,
                                    $rootScope,
                                    $element,
                                    d3,
                                    ...
                                    _,
                                   Stats) {
    console.log('organizationsByAssertionActivity loaded.');
    ...
    $scope.$watch(function() {
      ...
      chart.data = _.map(data, function(value, key) {
        return {
          Organization: key,
          Count: _.reduce(value.assertion_count, function(total, value, key) {
            return total + value;
          }, 0)
        };
      });
